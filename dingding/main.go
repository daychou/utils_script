...
func init() {
	...
	flag.StringVar(&msgInfo.To, "to", "manager3449", "消息的接收人，可以在钉钉后台查看，可空。")
	flag.StringVar(&msgInfo.Agentid, "agentid", "171925448", "AgentID，可以在钉钉后台查看，不可空。")
	...
	flag.StringVar(&msgInfo.Msg, "msg", `{ "from": "golang", "time": "2019.07.29 17:00:05", "level": "Warning", "name": "这是ZABBIX钉钉报警插件。", "key": "icmpping", "value": "30ms", "now": "56ms", "id": "1637", "ip": "8.8.8.8", "color":"FF4A934A", "age":"4m", "recoveryTime":"2019.07.29 17:03:05", "status":"OK" }`, "Json格式的文本消息内容，不可空。")
...
func getToken(corpid, corpsecret string) (token string) { //根据id和secret获取AccessToken
	...
	if err1 == nil {
		...
		log(fmt.Sprintf("AccessToken获取失败，%s", m.Errmsg))
		os.Exit(1)
	}
	log("Token解析失败！")
	os.Exit(1)
